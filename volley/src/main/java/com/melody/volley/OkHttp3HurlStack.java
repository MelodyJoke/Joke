...
/**
 * description: http stack for ok http 3.x
 * author: Melody
 * date: 2016/6/17
 * version: 0.0.0.1
 */
@SuppressWarnings("WeakerAccess, unused, deprecation")
public final class OkHttp3HurlStack implements HttpStack {

    private final OkHttpClient mClient;

    public OkHttp3HurlStack(OkHttpClient client) {
        this.mClient = client;
    }
    ...
    private static HttpEntity entityFromOkHttpResponse(Response response) throws IOException {
        ...
        if (body.contentType() != null) entity.setContentType(body.contentType().type());
    ...
    public HttpResponse performRequest(Request<?> request, Map<String, String> additionalHeaders)
        ...
        for (int i = 0, len = responseHeaders.size(); i < len; i++) {
            ...
            if (name != null) response.addHeader(new BasicHeader(name, value));
        }
